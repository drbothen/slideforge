...
//!
//! Produces an SVG area chart from an [`InternalChartSpec`] using the
//! `plotters` `SVGBackend` with `AreaSeries`.
...
/// Render an area (filled line) chart to an SVG string.
///
/// # Errors
///
/// Returns [`ChartError`] if the data is invalid or the plotters backend
/// fails to produce output.
pub(crate) fn render_area(spec: &InternalChartSpec) -> Result<String, ChartError> {
    ...
    let mut svg_buf = String::new();
    {
        ...
        for (series_idx, series) in spec.data.iter().enumerate() {
            ...
            // Create a semi-transparent fill color for the area.
            let fill_color = color.mix(0.3);
            ...
            chart
                .draw_series(AreaSeries::new(data.clone(), 0.0, fill_color))
                .map_err(|e| ChartError::RenderError {
                    message: Arc::from(e.to_string().as_str()),
                })?;
