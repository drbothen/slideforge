...
//!   is invoked. The `validation::data_is_empty(&Value)` helper provides
//!   a precondition check at the raw `Value` layer; pipeline-level detection
//!   is handled by `ChartEmptyDataValidator` in `slideforge-validate`.
