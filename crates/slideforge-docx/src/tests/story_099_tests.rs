...
use slideforge_layout::sections::{
...
/// Build a [`LaidOutDeck`] that wraps the provided slides and sections, using
...
// ─── F-099-001 [HIGH]: Section run universality — all section runs carry <w:lang> ──

/// F-099-001 / BC-5.01.005 PC-6 — section run lang universality:
///
/// A deck with at least one auto-generated section (ExecutiveSummary) AND one
/// manually-authored section must produce `word/document.xml` where every
/// `<w:r>` run carries a corresponding `<w:rPr><w:lang>` element.
///
/// This is the UNIVERSALITY check: the count of `<w:r>` open-elements must
/// equal the count of `<w:lang` occurrences. No run — whether in main body
/// paragraphs, section heading paragraphs, or section content paragraphs —
/// may omit its `<w:lang>` annotation.
///
/// Pre-fix defect: `auto_sections.rs::make_styled_paragraph` and
/// `manual_sections.rs::make_styled_paragraph` constructed `Run` values with
/// no `RunProperties`, so the section runs were the only runs in the document
/// that lacked `<w:lang>`.
#[test]
fn test_BC_5_01_005_section_runs_have_lang_universality() {
    let deck = deck_with_lang(Some("en"));

    // One auto-generated ExecutiveSummary section with two takeaway bullets.
    let auto_section = GeneratedSection {
        kind: SectionKind::ExecutiveSummary,
        source: SectionSource::AutoGenerated,
        items: vec![
            SectionItem::TakeawayBullet(Arc::from("Takeaway one")),
            SectionItem::TakeawayBullet(Arc::from("Takeaway two")),
        ],
    ...
    // One manually-authored section with a report-register content paragraph.
    let manual_section = GeneratedSection {
        kind: SectionKind::ManualSection(Arc::from("methodology")),
        source: SectionSource::ManuallyAuthored,
        items: vec![],
        heading: Arc::from("Methodology"),
        target_formats: vec![OutputFormat::Docx, OutputFormat::Pdf],
        register_content: vec![RegisteredContent::plain(
            Register::Report,
            Arc::from("Manual section content paragraph."),
        )],
    };

    // A regular slide so the deck is non-empty.
    let slide = make_slide_with_bullets("Section Universality Slide", &["Body bullet"]);

    let laid_out =
        make_laid_out_deck_with_sections(vec![slide], vec![auto_section, manual_section]);
    ...
    assert!(
        run_count > 0,
        "F-099-001 universality precondition: at least one <w:r> must exist"
    );
    assert_eq!(
        run_count,
        lang_count,
        "F-099-001 RED GATE [HIGH]: every <w:r> — including runs inside auto-generated \
         and manual section paragraphs — must carry <w:lang> in its <w:rPr>. \
         Found {run_count} <w:r> elements but only {lang_count} <w:lang> elements. \
         Section heading and content runs emitted by auto_sections.rs and \
         manual_sections.rs must carry RunProperties with Languages \
         (mirrors make_lang_run in document_body.rs). \
         Got (first 5000 chars):\n{}",
        &doc_xml[..doc_xml.len().min(5000)]
    );
}

// ─── OBS-099-001 [MED]: No-lang docProps default untested — PC-6 cross-surface ──

/// OBS-099-001 / BC-5.01.005 PC-6 — no-lang deck uses canonical default `"en"` on
/// BOTH surfaces simultaneously:
///
/// A deck with no `lang` declaration must produce:
///   1. `docProps/core.xml`: `<dc:language>en</dc:language>` (exactly `"en"`, not `"en-US"`)
///   2. `word/document.xml`: `<w:lang w:val="en"` (exactly `"en"`, not `"en-US"`)
///
/// This is a CROSS-SURFACE identity test (BC-5.01.005 PC-6): both surfaces must
/// use the same canonical default — `DEFAULT_DECK_LANG = "en"` — and neither
/// may diverge to `"en-US"` independently.
#[test]
fn test_BC_5_01_005_no_lang_docprops_default_en_cross_surface() {
    // No lang declared → both surfaces must default to "en".
    let deck = deck_with_lang(None);
    let rc = RegisteredContent::plain(
        Register::Report,
        Arc::from("Report paragraph for lang test"),
    );
    let mut slide = make_slide_with_bullets("No-Lang Deck Slide", &[]);
    slide.register_content = vec![rc];
    let laid_out = make_laid_out_deck(vec![slide]);
    ...
    // ── Surface 1: docProps/core.xml must carry <dc:language>en</dc:language> ─
    assert!(
        core_xml.contains("<dc:language>en</dc:language>"),
        "OBS-099-001 RED GATE: docProps/core.xml must contain <dc:language>en</dc:language> \
         (exactly \"en\", NOT \"en-US\") when no lang is declared \
         (BC-5.01.005 PC-6 / DEFAULT_DECK_LANG = \"en\"). \
         Got core.xml:\n{core_xml}"
    );

    // Must NOT contain "en-US" in the dc:language element.
    assert!(
        !core_xml.contains("<dc:language>en-US</dc:language>"),
        "OBS-099-001 RED GATE: docProps/core.xml must NOT contain <dc:language>en-US</dc:language> \
         when no lang is declared; canonical default is \"en\" per BC-5.01.004. \
         Got core.xml:\n{core_xml}"
    );

    // ── Surface 2: word/document.xml must carry <w:lang w:val="en" ───────────
    assert!(
        doc_xml.contains("<w:lang w:val=\"en\"") || doc_xml.contains("<w:lang w:val='en'"),
        "OBS-099-001 RED GATE: word/document.xml must carry <w:lang w:val=\"en\"/> \
         (NOT \"en-US\") when no lang is declared (BC-5.01.005 PC-4 / PC-6 cross-surface). \
         Got (first 3000 chars):\n{}",
        &doc_xml[..doc_xml.len().min(3000)]
    );
    ...
    assert!(
        !doc_xml.contains("<w:lang w:val=\"en-US\""),
        "OBS-099-001 RED GATE: word/document.xml must NOT carry <w:lang w:val=\"en-US\"/> \
