...
/// manual sections first, then auto-generated, unless `section_order:` in the
...
#[must_use]
pub fn collect_sections(_deck: &Deck) -> Vec<GeneratedSection> {
    todo!(
        "STORY-027: collect_sections — combine manual and auto-generated sections, \
         apply supersession rule (AC-006), apply section_order sorting (AC-007)"
    )
}
...
#[must_use]
pub fn collect_executive_summary(_deck: &Deck) -> Option<GeneratedSection> {
    todo!(
        "STORY-027: collect_executive_summary — iterate deck.slides, collect \
         takeaway fields → SectionItem::TakeawayBullet, return None if empty (AC-003)"
    )
}
...
#[must_use]
pub fn collect_risk_register(_deck: &Deck) -> Option<GeneratedSection> {
    todo!(
        "STORY-027: collect_risk_register — iterate deck.slides for severity_cards type, \
         collect card entries → SectionItem::RiskRow, return None if empty (AC-003)"
    )
}
...
mod tests {
    ...
    use slideforge_types::{
        Deck, DeckMetadata, FieldValue, OrderedMap, Slide, SourceSpan, Value,
    };
    ...
    fn make_severity_card_slide(title: &str, severity: &str, description: &str, owner: &str) -> Slide {
        let mut fields = OrderedMap::new();
        fields.insert(
            Arc::from("title"),
