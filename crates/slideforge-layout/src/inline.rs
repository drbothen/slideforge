...
///   AND by `ContentBlock::Bullets` (STORY-073 / AC-001). Each `BulletItem`
///   produces exactly one `FrameContent::TextRun` frame carrying
///   `BulletItem.inlines` verbatim (parent before children, depth-first).
///   Bullet-produced `TextRun` frames are scanned here exactly like text-block
///   frames — xref targets are validated and inline depth is bounded.
