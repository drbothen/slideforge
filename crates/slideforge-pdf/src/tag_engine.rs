...
impl SlideTagEngine {
    ...
    /// # Panics
    ///
    /// Does not panic in practice. The `expect` on `NonZeroU16::new(2)` is
    /// infallible because 2 is a valid non-zero value; the expect message
        ...
        for (frame_idx, frame) in slide.frames.iter().enumerate() {
            match &frame.content {
                ...
                FrameContent::Subtitle(_text) => {
                    // SAFETY: 2 is a valid non-zero value.
                    // NonZeroU16::new(2) returns Some(_) — expect is infallible.
                    let level = std::num::NonZeroU16::new(2)
                        .expect("2 is a valid NonZeroU16 value — infallible");
                    let heading_group =
                ...
                FrameContent::Image { alt } => {
                    if alt.is_empty() {
                        ...
                        let figure_group = TagGroup::new(
                            Tag::<krilla::tagging::kind::Figure>::Figure(Some(alt.to_string())),
                        );
                ...
                FrameContent::Diagram(diagram_svg) => {
                    ...
                    let alt_text = if diagram_svg.as_str().is_empty() {
                        ...
                        Some(String::from("diagram"))
                    };
                    let figure_group =
                        TagGroup::new(Tag::<krilla::tagging::kind::Figure>::Figure(alt_text));
                    part_group.push(figure_group);
                },
                ...
                FrameContent::Chart => {
                    ...
                    let figure_group = TagGroup::new(Tag::<krilla::tagging::kind::Figure>::Figure(
                        Some(String::from("chart")),
                    ));
                    part_group.push(figure_group);
                },
    ...
    fn tag_content_block(
        ...
        match block {
            ...
            ContentBlock::Shape(shape_spec) => match &shape_spec.alt {
                ...
                Some(AltText::Provided(alt)) => {
                    let figure_group = TagGroup::new(Tag::<krilla::tagging::kind::Figure>::Figure(
                        Some(alt.to_string()),
                    ));
                    Ok(Some(figure_group))
                },
            },
            ...
            ContentBlock::Chart(chart_spec) => {
                let alt = chart_spec
                    ...
                    .unwrap_or_else(|| String::from("chart"));
                let figure_group =
                    TagGroup::new(Tag::<krilla::tagging::kind::Figure>::Figure(Some(alt)));
                Ok(Some(figure_group))
            },
    ...
    /// Build a `Table → TR → TH/TD` tag group structure from a [`TableSpec`].
    ///
    /// Produces:
    /// ```text
    /// Table
