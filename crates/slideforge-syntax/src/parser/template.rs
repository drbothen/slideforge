...
pub(super) enum InlineMarkupRoute {
    ...
    ///
    /// Field: `scheme` — the disallowed scheme string (e.g. `"javascript"`, `"(none)"`).
    ///
    /// The user-facing error message is generated by `SyntaxError::DisallowedLinkUrlScheme`'s
    /// `#[error(...)]` attribute (in `error.rs`) rather than from a routing-tag `clean_msg`.
    /// The `clean_msg` field was previously threaded here but always discarded — removed in
    /// OBS-FU-P1-B cleanup to eliminate the dead-field confusion.
    DisallowedLinkUrlScheme(String),
}
...
pub(super) fn parse_routing_tag(msg: &str) -> Option<InlineMarkupRoute> {
    ...
    match kind_str {
        ...
        // `clean_msg` is not forwarded — the user-facing message comes from the
        // `#[error(...)]` attribute on `SyntaxError::DisallowedLinkUrlScheme` (OBS-FU-P1-B).
        "DisallowedLinkUrlScheme" => Some(InlineMarkupRoute::DisallowedLinkUrlScheme(delim)),
        _ => None,
    }
}
...
fn scan_template_chunks(
    ...
    while pos < len {
            ...
            && s[pos..].starts_with(close)
        {
            ...
            if close == "_" {
                // OBS-P24-A / F-FU-P1-001: use the shared is_right_flanking_underscore
                // helper so the close-guard and the opener's pre-scan are always in sync.
                if is_right_flanking_underscore(bytes, pos) {
            ...
            if let Some(bracket_close_rel) = s[pos + 1..].find("](") {
                ...
                if let Some(paren_close_rel) = s[url_start..].find(')') {
                    ...
                    if !ALLOWED_LINK_SCHEMES.contains(&scheme.as_str()) {
                        ...
                        // F-FU-P1-002: the link is syntactically well-formed (closing `)`
                        // was found at `paren_close`), so recovery must resume scanning
                        // at `paren_close + 1` — NOT terminate at `pos = len`.
                        // error-taxonomy v2.13 E-PAR-022 Note: "accumulated (does not
                        // halt parsing — remaining field content continues to be parsed)."
                        // Emit the disallowed link's span as a literal for output continuity.
                        let link_literal = &s[link_open_pos..=paren_close];
                        if !link_literal.is_empty() {
                            chunks.push(TemplateChunk::Literal(link_literal.to_string()));
                        }
                        pos = paren_close + 1;
                        lit_start = pos;
                        continue;
...
/// Returns `true` when the `_` at byte position `pos` in `bytes` is a
/// **right-flanking delimiter** — i.e., it is immediately followed by an ASCII
/// alphanumeric character or another `_`.
///
/// A right-flanking `_` is word-internal and must NOT act as an italic closer
/// (OBS-P24-A close-guard) or be counted as a valid closer when the italic
/// opener is pre-scanning for a match (F-FU-P1-001 fix).
///
/// This is the **single source of truth** for the right-flanking predicate used
/// by BOTH the close-guard in the recursive scanner loop AND the opener's
/// `has_valid_italic_closer` pre-scan.  Keeping both sites in sync via this
/// function prevents future drift (TD-VSDD-060).
///
/// # Arguments
///
/// * `bytes` — the full field-value byte slice being scanned.
/// * `pos` — the byte index of the `_` to test (must be in bounds).
fn is_right_flanking_underscore(bytes: &[u8], pos: usize) -> bool {
    bytes
        .get(pos + 1)
        .is_some_and(|b| b.is_ascii_alphanumeric() || *b == b'_')
}

/// Returns `true` when `rest` (the content after an opening `_`) contains at
/// least one `_` that is **not** right-flanking — i.e., a `_` that could act
/// as a valid italic closer under the OBS-P24-A bilateral flanking guard.
///
/// This is used by the open-`_` branch (F-FU-P1-001 fix) to detect whether a
/// valid closer actually exists before opening an italic span.  It uses the
/// same [`is_right_flanking_underscore`] predicate as the close-guard so the
/// two never drift apart.
///
/// # Arguments
///
/// * `rest` — the slice of the field value starting immediately after the
///   opening `_` (i.e., the content that will be recursively scanned).
fn has_valid_italic_closer(rest: &str) -> bool {
    let bytes = rest.as_bytes();
    for (i, &b) in bytes.iter().enumerate() {
        if b == b'_' && !is_right_flanking_underscore(bytes, i) {
            return true;
        }
    }
    false
