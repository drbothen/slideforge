...
updated: 2026-06-08 (STORY-079 merged, develop 2f6d5da4)
---
...
2. Verify `git rev-parse develop` == `git rev-parse origin/develop`. Canonical SHA in STATE.md frontmatter (`develop_sha`). Current: `2f6d5da4` (72 merged PRs).
...
5. Confirm workspace tests green: `cargo nextest run --workspace --no-fail-fast` (expect ~3690+ pass, 0 fail, 18 skip; cold_budget STORY-080 converged, PR pending).
...
| FU-079-SEC-001 | STORY-079 MEDIUM (Phase 6): add a compile-time exhaustiveness check on usvg `Node` variants so a future usvg upgrade adding a container Node can't silently bypass the SEC-001 nesting-depth guard. The iterative DFS matches on explicit variants — a new container variant would fall through to the base case and not count depth. | pending | medium (non-blocking) | Anchored to Phase 6 formal-hardening / diagrams-security-hardening story; PR #72 security review. |
| FU-079-SEC-LOW | STORY-079 LOW (Phase 6, composite): (a) truncate `slide_title` in DoS-guard error messages to min(256 chars) to bound amplification; (b) define MAX_SVG_BYTES/MAX_SVG_NESTING_DEPTH as shared constants in one crate (not duplicated across slideforge-diagrams + slideforge-pdf) with a compile-time parity assert; (c) ensure distinct sentinel spans per guard error path (byte-size vs depth guard emit distinguishable source locations). | pending | low | Anchored to Phase 6 hardening; PR #72 security + pr-reviewer review. |
| FU-079-DOC-NITS | STORY-079 cosmetic (pr-reviewer): normalize.rs AC-003 doc comment calls `simple_geometry_svg` a "Mermaid flowchart" (it is synthetic geometry SVG); evidence-report.md edge-case IDs use EC-001..006 while BC-1.12.003 v1.3 enumerates EC-005..008. Non-behavioral. | pending | low (OBS) | Anchored to doc sweep or next diagrams touch; PR #72 pr-reviewer finding. |
| FU-080-CI-PLATFORMS | STORY-080: AC-001 + CLAUDE.md Quality Bar require 5 CI platforms including both macOS architectures (macos-13 + macos-14); ci.yml currently has 4 legs (macos-latest only). 5th macOS leg should be added. Non-blocking for STORY-080 merge. | pending | low | Anchored to a CI/devops story or wave-gate; discovered during STORY-080 adversary cascade. |
| FU-080-BC-PROSE | STORY-080: story body (~line 35) attributes cold_budget to "BC-1.12.003 normalization pipeline" while frontmatter anchors BC-1.03.002 — minor pre-existing subsystem-justification prose inconsistency. Non-behavioral. | pending | low (OBS) | Anchored to story cleanup sweep; non-blocking. |
| FU-CROSS-BC-VERSION-DRIFT | STORY-028 (cites BC-3.04.001 v1.5.2 in story body) and STORY-072 (cites v1.3) carry stale BC-version cells vs actual BC-3.04.001 v1.6. Recurred as a finding at STORY-074 pass 9+. | pending | low | Anchored to wave-gate cross-story version sweep; recurred 3+ times. |

---

## Process Gaps (S-7.02 cycle-closing; [process-gap])

| id | description | recurrence | proposal | anchor |
|----|-------------|------------|----------|--------|
| PROC-GAP-BC-VERSION-SYNC | No automated check that a story-body BC-version cell matches the referenced BC's frontmatter `version` field. The state-manager and consistency-validator both rely on manual inspection. Recurred 3+ times: STORY-074 cascade passes 9+, STORY-028, STORY-072 (see FU-CROSS-BC-VERSION-DRIFT). | 3+ recurrences | Add a consistency-validator axis (or a pre-commit lint hook) that diffs story BC-table version cells against BC frontmatter `version`. | Anchored to a self-improvement / tooling story; Phase 3 post-wave-5-gate. |
| PROC-GAP-WORKTREE-CWD-LEAK | Subagents writing into the MAIN repo working tree when a bash command is not prefixed with `cd <worktree>` — the shell does NOT persist `cd` across separate bash tool calls (each call resets to project root cwd). Caused STORY-081 and STORY-082 stub files + build artifacts to leak into the main working tree (since cleaned). | 2 confirmed occurrences (STORY-081/082) | Per-story-delivery agent prompts must mandate `cd <worktree> &&` on EVERY bash command, or use absolute paths throughout. Consider a dispatcher rule that blocks production-source commits from within a factory-management context. | Anchored to self-improvement story or orchestrator dispatch-template rule update. |
| PROC-GAP-SPECFIX-DEVELOP-COMMIT | A spec-fix dispatch (architect, ADR-021 OBS-2 investigation) committed a workspace Cargo.toml change DIRECTLY to develop rather than confining changes to `.factory/`, causing a develop divergence (since reset). Violates LESSON-12. | 1 confirmed occurrence (ADR-021 OBS-2) | Spec-fix dispatches (architect/PO/story-writer) must be explicitly scoped to `.factory/` ONLY, forbidden from committing to develop or touching `crates/`. Orchestrator dispatch templates must include an explicit prohibition. | Anchored to self-improvement story / orchestrator dispatch-template rule. |
...
- **STORY-079 PR #72** (2f6d5da4, 2026-06-08) — slideforge-diagrams SVG DoS hardening. SEC-001 nesting-depth guard (CWE-674, iterative DFS cap 64), SEC-002 byte-size guard (CWE-400, cap 50 MiB). E-EXP-004 added to error taxonomy. BC-1.12.003 v1.3 (inv-4 ordering, PC-8/9/10, EC-005..008). error-taxonomy v2.26 (E-EXP-004 sibling-accuracy). export-architecture v1.4 (root-at-1 iterative). LOCAL adversary 3/3 strict-CLEAN (passes 3-4-5; 5 total). Security CLEAN. pr-reviewer APPROVE. CI green (cold_budget flake cleared on re-run). Follow-ups: FU-079-SEC-001 (MEDIUM, Phase 6 exhaustiveness), FU-079-SEC-LOW, FU-079-DOC-NITS (all anchored Phase 6 / next diagrams touch).
