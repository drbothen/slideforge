...
wave_5_status: "10 of 22 Wave-5 stories MERGED (…STORY-072 PR#76, STORY-082 PR#77, STORY-088 PR#78). 12 stories remain. In-flight (1 active worktree): STORY-081 (adversary 0/3 — Pass 6 NOT CLEAN HEAD 577767ec; Pass-5 ResolvedFace fixes CONFIRMED load-bearing; NEW: ADV-P06-MED-001 BoldItalic measure/draw fallback-chain divergence; cold_budget adjudicated OUT of STORY-081 scope; NEXT: implementer fix burst face_for_span_kind helper then Pass 7). CI infra fix PR#79 merged."
develop_sha: "15838de1"
develop_pr_count: 79
error_taxonomy_version: "v2.28"
workspace_tests: "3759 pass / 20 skip (STORY-081 worktree 577767ec; Pass-5 ResolvedFace fix burst: MED-001 + OBS-P05-001 closed by type invariant; linux-arm64 CI: 1 unresolved nextest failure — FU-CI-ARM64-TEST-FAILURE)"
...
- **Worktree:** `.worktrees/STORY-081` | **Branch:** `feature/STORY-081` | **HEAD:** `577767ec` (Pass-5 ResolvedFace fix burst — MED-001 + OBS-P05-001 closed by type invariant — completed; now checkpointed; tests 3759 pass)
- **Adversary streak:** **0/3** — Pass 6 NOT CLEAN (1 MED + 2 OBS, zero CRIT/HIGH). See pass report `cycles/STORY-081/adversarial-reviews/adversary-STORY-081-pass-6.md`.
- **Pass 5 findings (ALL CONFIRMED LOAD-BEARING, CLOSED):** ADV-P05-MED-001 — `ResolvedFace{font, raw, face_index}` (font.rs:69-95) co-locates all three; `resolve_styled_face_via_fontdb` (font.rs:455-497) captures `face_index` from `with_face_data` and builds krilla Font at SAME index; `compute_multi_span_x_positions` (exporter.rs:1224-1264) passes `face.face_index` (not 0) to `measure_text_width_pt`; load-bearing test `test_adv_p05_med001_obs_p05_001_resolved_face_carries_face_index` (inline_markup_pdf_snapshot.rs:700-829) asserts measure differs by face index. OBS-P05-001 — type invariant makes `Some(font)+None raw` unrepresentable; all constructor/access sites swept (LESSON-19), compile-checked against new shape.
- **Pass 6 findings (ALL OPEN):**
  - ADV-P06-MED-001[MED] OPEN — `compute_multi_span_x_positions` (exporter.rs:1243-1245) `Bold|BoldItalic => bold.or(regular)` vs `font_for_span` (exporter.rs:1294-1299) `BoldItalic => bold.or(italic).or(regular)`. When `bold=None` & `italic=Some` (reachable — fontdb queries bold/italic independently; system can have italic-not-bold), measure uses REGULAR while draw uses ITALIC → different glyph advances → cursor mis-spacing. Reachable via EC-003 `**_bold italic_**`. All current fixtures populate bold so the `bold=None` path is never exercised. Fix: extract single `face_for_span_kind(span_face, font_set) -> Option<&ResolvedFace>` helper called from BOTH paths; align BoldItalic fallback to `bold→italic→regular`; add `bold=None/italic=Some` BoldItalic regression test asserting measured advance uses SAME face as draw path.
  - ADV-P06-OBS-001[OBS] OPEN — Full suite not executed (adversary read-only profile). `cold_budget.rs` NOT in STORY-081 diff (STORY-081 = eval/layout/pptx/docx/pdf/html, not diagrams); STORY-081 `load_system_fonts()` calls do NOT feed the diagrams `FONT_DB` OnceLock — NO causal link. Adjudicated OUT of STORY-081 perimeter; route to diagrams crate owner; should NOT block STORY-081 convergence.
  - ADV-P06-OBS-002[OBS, process-gap] OPEN — Wall-clock timing gate is a structural false-green/false-fail vector. STORY-081 adds double `load_system_fonts()` per PDF export (resolve_font_set + resolve_regular_face = two full scans); slideforge-pdf has no call-count gate. Double-scan is correct-but-slow (ADR-023-deferred OnceLock); zero-risk refactor would halve cold cost. Lessons-codification: pair/replace timing-threshold tests with deterministic call-count assertion.
- **fontdb pin:** =0.23.0 direct pin retained; cargo deny PASS.
- **Workspace tests at HEAD 577767ec:** 3759 pass / 20 skip.
- **NEXT ACTION:** Implementer fix burst — extract single `face_for_span_kind` helper used by BOTH `compute_multi_span_x_positions` + `font_for_span`; align BoldItalic fallback (`bold→italic→regular`); add `bold=None/italic=Some` BoldItalic regression test (measure face == draw face); OPTIONAL zero-risk: reuse fontdb `db` across `resolve_font_set`/`resolve_regular_face` (halves double-scan). Then LESSON-21 exit gate → adversary Pass 7.
...
- **FU-DIAGRAMS-COLD-BUDGET-TIMING-GATE** (HIGH — must resolve before STORY-081 PR): `crates/slideforge-diagrams/tests/cold_budget.rs` is a flaky wall-clock timing gate (likely the same flake as FU-CI-ARM64-TEST-FAILURE; 300ms budget, 242ms observed = 19% headroom). Convert to a deterministic `load_count == 1` assertion OR widen/remove the wall-clock bound. Full-suite CI would flake on STORY-081 PR if not resolved first. Linked to FU-CI-ARM64-TEST-FAILURE and ADV-P06-OBS-002 lessons-codification item.
- **FU-088-BC10102-ANCHOR** (spec-steward; non-blocking): pre-existing BC-1.01.002 H1 title/anchor mismatch.
- **FU-EXIT-GATE-DISTINGUISHING-OUTPUT** (process-improvement; source OBS-1 Pass-3 + OBS-P04-001 Pass-4 + OBS-P05-001 Pass-5 + ADV-P06-MED-001 Pass-6 adversary STORY-081): Anti-pattern recurred 6th time (text-drop → face-drop → position-collapse → drawable-font/unmeasurable-raw seam → measure/draw slot divergence). Exit-gate rule must verify (1) non-test caller consumes a dispatch fn's DISTINGUISHING output; (2) multi-span-same-line positional assertion; (3) when drawable font present but raw bytes absent, non-silent diagnostic (`tracing::warn` or type-invariant enforcement); (4) BOTH measure path AND draw path select slots via THE SAME helper function (shared `face_for_span_kind`). Target: lessons-codification / Standing Process Rules. Tracked for next session-wrap or lessons update.
...
- Active worktrees: 1 — STORY-081 in `.worktrees/STORY-081` on `feature/STORY-081` HEAD `577767ec`. Pass 6 NOT CLEAN (1 MED + 2 OBS, zero CRIT/HIGH). Pass-5 ResolvedFace fixes CONFIRMED load-bearing. Implementer fix burst (face_for_span_kind helper) NEXT. Open PRs: 0.
- Workspace: 3759 pass / 20 skip (STORY-081 worktree at HEAD 577767ec; cargo deny PASS; cold_budget PERMANENTLY FIXED by STORY-080 PR#73; linux-arm64 CI has 1 unresolved nextest failure — FU-CI-ARM64-TEST-FAILURE).
...
6. **Continue in priority order:** STORY-081 — Pass 6 NOT CLEAN (HEAD `577767ec`; 1 MED + 2 OBS, zero CRIT/HIGH). Pass-5 ResolvedFace fixes (MED-001 + OBS-P05-001) CONFIRMED load-bearing. Implementer fix burst: extract single `face_for_span_kind(span_face, font_set) -> Option<&ResolvedFace>` helper called from BOTH `compute_multi_span_x_positions` AND `font_for_span`; align BoldItalic fallback to `bold→italic→regular`; add `bold=None/italic=Some` BoldItalic regression test asserting measured advance uses SAME face as draw path (measure face == draw face, i.e. italic not regular); OPTIONAL zero-risk: reuse fontdb `db` across `resolve_font_set`/`resolve_regular_face` (halves double-scan). Then LESSON-21 exit gate → adversary Pass 7. Pass 6 report: `cycles/STORY-081/adversarial-reviews/adversary-STORY-081-pass-6.md`. cold_budget diagrams flake adjudicated OUT of STORY-081 scope — must be resolved before STORY-081 PR (FU-DIAGRAMS-COLD-BUDGET-TIMING-GATE). Per-story adversary passes SERIAL (LESSON-7 + rate-limit).
...
**Wave 5 IN PROGRESS. develop 15838de1 (79 merged PRs). STORY-081 Pass 6 NOT CLEAN — implementer fix burst (face_for_span_kind helper) NEXT. 1 worktree active. 12 stories remain.**
...
| **Active worktrees** | 1 — STORY-081 in `.worktrees/STORY-081` on `feature/STORY-081` HEAD `577767ec`. |
| **STORY-081 state** | HEAD `577767ec` (Pass-5 ResolvedFace fix burst — MED-001 + OBS-P05-001 closed by type invariant — completed; now checkpointed); adversary 0/3; Pass 6 NOT CLEAN (1 MED + 2 OBS, zero CRIT/HIGH); Pass-5 findings ALL CONFIRMED load-bearing |
| **Pass 6 open findings** | ADV-P06-MED-001: BoldItalic measure path `bold.or(regular)` vs draw path `bold.or(italic).or(regular)`; when `bold=None` & `italic=Some`, measure selects REGULAR, draw selects ITALIC → cursor mis-spacing; reachable EC-003 `**_bold italic_**`. Fix: extract `face_for_span_kind` helper; align fallback `bold→italic→regular`; add `bold=None/italic=Some` regression test. ADV-P06-OBS-001: cold_budget adjudicated OUT of STORY-081 scope (pre-existing diagrams flake; FU-DIAGRAMS-COLD-BUDGET-TIMING-GATE must be resolved before PR). ADV-P06-OBS-002: double `load_system_fonts()` per PDF export; lessons-codification for timing-gate → call-count-assertion pattern. |
| **Workspace tests** | 3759 pass / 20 skip (HEAD 577767ec; cargo deny PASS; linux-arm64 CI: 1 unresolved nextest failure — FU-CI-ARM64-TEST-FAILURE) |
| **factory-artifacts** | Pushed to origin. Fresh sessions: clone + `git worktree add .factory factory-artifacts`. |
| **RESUME INSTRUCTION** | STORY-081 Pass 6 NOT CLEAN (HEAD 577767ec). NEXT: implementer fix burst — extract single `face_for_span_kind(span_face, font_set) -> Option<&ResolvedFace>` helper called from BOTH `compute_multi_span_x_positions` AND `font_for_span`; align BoldItalic fallback to `bold→italic→regular`; add `bold=None/italic=Some` BoldItalic regression test asserting measured advance uses SAME face as draw path; OPTIONAL zero-risk: reuse fontdb `db` across `resolve_font_set`/`resolve_regular_face` (halves double-scan). LESSON-21 exit gate → adversary Pass 7. PRE-PR BLOCKER: FU-DIAGRAMS-COLD-BUDGET-TIMING-GATE (cold_budget.rs flaky wall-clock gate must be converted to call-count==1 OR widened before full-suite CI on STORY-081 PR). Pass 6 report: `cycles/STORY-081/adversarial-reviews/adversary-STORY-081-pass-6.md`. Per-story: LOCAL adversary 3-CLEAN (SEQUENTIAL) → demo-recorder → rebase onto 15838de1 → pr-manager 9-step → STANDING MERGE AUTH → squash-merge → state-manager post-merge burst → worktree cleanup. Rate-limiting: ONE adversary/review pass at a time. On next PR: CAPTURE linux-arm64 nextest failure test name (FU-CI-ARM64-TEST-FAILURE). HELD next batch: STORY-056/048 (unblocked ←047), STORY-057/058/064 (serialize cli), STORY-060/061 (GIT2-OPENSSL first). |
