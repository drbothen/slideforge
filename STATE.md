...
| **Open PRs** | 0 |
| **Workspace crates** | 16 |
| **Spec deltas this session** | STORY-084 delivered: 7 bundled SectionType impls in slideforge-plugin-api. BC-5.02.001 SectionType surface closed. BC-3.02.001 invariant 4 (notes-exclusion) enforced on both auto types. LESSON-17 added: inverted-Red-Gate anti-pattern. // STORY-085 adversary round 1 (6 findings fixed): F-001 ADR-017 Option A, BC-5.02.001 → v1.4; F-002 AC-005 paper-fix corrected; F-003 byte-identity 12-variant; F-004 HTML escaping BC-3.05.001 → v1.3.6 EC-009/EC-010, VP-053 created; F-005 depth guard; F-006 STORY-049 anchor; F-008 module-boundary fitness. BC-5.02.002 → v1.4. ADR-017 accepted. // STORY-085 adversary round 2: 6 prior findings verified closed; new findings fixed: combined run-property accumulator (no silent drop of nested formatting in any direction, commit d9daad0a), stale docstring corrected, depth guard threaded through render_with_context link path. Spec: AC-006 prose split by variant group (Plain/Bold/Italic byte-identical, 6 richer variants), VP-053 harness field fix (MathNode::inline latex/display/span). HIGH-1 doc-gate prediction was FALSE POSITIVE (cargo doc passes; private-item links unchecked). Convergence: still 0/3 strict-CLEAN (pass 2 had findings; pass 3 pending). // STORY-085 adversary round 3: 3 MED footnote-variant findings fixed in code; tracing-test dev-dep added to story spec Library table (TD-VSDD-059 load-bearing). OBS-4 process-gap captured. Code HEAD 5a99962f; 3161 tests pass. // STORY-085 adversary round 4: all prior findings re-verified closed. I-1 (ADR-017 over-claimed F-006 scope) reconciled via ADR Amendment Log — Question 3 + Consequence #4 + Decision table updated to reflect two-story split (STORY-085=registry-ready, STORY-049=live-resolution). OBS-1 (empty-text Link orphan-rel) fixed in code (75bd321d, rId-count-invariant tests). Code HEAD 75bd321d; 3163 tests pass. Convergence: still 0/3 strict-CLEAN (pass 4 had findings; pass 5 pending). // STORY-085 adversary round 5: all prior findings re-verified closed. F-P5-001 + OBS-P5-001 (orphan-rel drift from non-empty Vec flattening to empty display text) fixed structurally — shared pub fn display_text_is_empty in slideforge-types/src/inline.rs, single SoT for both rId-guard (notes_slide.rs) and hlinkClick-guard (default_formatter.rs). Story File Structure table updated (slideforge-types/src/inline.rs row added). Code HEAD 4283a17c; 3172 tests pass. Convergence: still 0/3 strict-CLEAN (pass 5 had findings; pass 6 pending). |
