...
wave_5_status: "10 of 22 Wave-5 stories MERGED (…STORY-072 PR#76, STORY-082 PR#77, STORY-088 PR#78). 12 stories remain. In-flight (1 active worktree): STORY-081 (adversary 0/3 — Pass 5 NOT CLEAN HEAD 46ed9eb9; Pass-4 findings ALL CONFIRMED load-bearing; Pass-4 fix burst HEAD 46ed9eb9 recorded; NEW: ADV-P05-MED-001 face_index=0 hardcode in width-measurement vs .ttc-resolved draw face + OBS-P05-001 drawable-font/unmeasurable-raw seam; NEXT: implementer fix burst ResolvedFace type-invariant then Pass 6). CI infra fix PR#79 merged."
develop_sha: "15838de1"
develop_pr_count: 79
error_taxonomy_version: "v2.28"
workspace_tests: "3759 pass / 20 skip (STORY-081 worktree 46ed9eb9; Pass-4 fix burst: CRIT-001 + HIGH-001 + super/sub size-reduction + docstrings; linux-arm64 CI: 1 unresolved nextest failure — FU-CI-ARM64-TEST-FAILURE)"
...
- **Worktree:** `.worktrees/STORY-081` | **Branch:** `feature/STORY-081` | **HEAD:** `46ed9eb9` (Pass-4 fix burst — CRIT-001 + HIGH-001 + size-reduction + docstrings — completed; uncheckpointed until this burst; now recorded)
- **Adversary streak:** **0/3** — Pass 5 NOT CLEAN (1 MED + 1 OBS, zero CRIT/HIGH). See pass report `cycles/STORY-081/adversarial-reviews/adversary-STORY-081-pass-5.md`.
- **Pass 4 findings (ALL CONFIRMED LOAD-BEARING, CLOSED):** ADV-P04-CRIT-001 — multi-span cursor advance via `measure_text_width_pt` (real cmap→hmtx); positional regression test strictly-increasing; "for now" comment gone. ADV-P04-HIGH-001 — `effective_span_font_size` returns `base * 0.583`; `adjusted_baseline_y` shifts `±base * 0.333`; slide_pdf.rs docstrings describe live `draw_text` mechanism. OBS-P04-001 — superseded by OBS-P05-001.
- **Pass 5 findings (ALL OPEN):**
  - ADV-P05-MED-001[MED] OPEN — `compute_multi_span_x_positions` (exporter.rs:1239) hardcodes `face_index=0` in `measure_text_width_pt` call; drawn krilla Font uses fontdb-resolved `face_index` (non-zero for .ttc collections); `ResolvedFontSet` stores only `raw_*` bytes, no per-face `face_index` — mismatched face → wrong glyph advances → mis-spaced multi-span lines. Strictly-increasing test passes (advances positive) but SPACING wrong. Fix: introduce `ResolvedFace { font, raw, face_index }` type invariant; thread per-span `face_index` into `compute_multi_span_x_positions`; add regression test asserting correct-index vs index-0 measure differs.
  - OBS-P05-001[OBS — process-gap] OPEN — `PdfExporter::with_resolved_font_set` seam allows `Some(font) + None raw` → ZERO cursor advance → silent overprint, no diagnostic. Codify: emit `tracing::warn` when drawable font but unmeasurable raw, OR enforce `ResolvedFace` pairing as type invariant (subsumes MED-001 root cause). Extends OBS-P04-001 / FU-EXIT-GATE-DISTINGUISHING-OUTPUT.
- **fontdb pin:** =0.23.0 direct pin retained; cargo deny PASS.
- **Workspace tests at HEAD 46ed9eb9:** 3759 pass / 20 skip.
- **NEXT ACTION:** Implementer fix burst — refactor `ResolvedFontSet` to `ResolvedFace { font, raw, face_index }` type invariant (closes BOTH ADV-P05-MED-001 + OBS-P05-001 structurally); thread per-span `face_index` into `compute_multi_span_x_positions` → `measure_text_width_pt` (exporter.rs:1239, not 0); add non-zero-face-index regression test (.ttc or synthetic unit test asserting measure differs by index). Then LESSON-21 exit gate → adversary Pass 6.
...
- **FU-EXIT-GATE-DISTINGUISHING-OUTPUT** (process-improvement; source OBS-1 Pass-3 + OBS-P04-001 Pass-4 + OBS-P05-001 Pass-5 adversary STORY-081): Anti-pattern recurred 5th time (text-drop → face-drop → position-collapse → drawable-font/unmeasurable-raw seam). Exit-gate rule must verify (1) non-test caller consumes a dispatch fn's DISTINGUISHING output; (2) multi-span-same-line positional assertion; (3) when drawable font present but raw bytes absent, non-silent diagnostic (`tracing::warn` or type-invariant enforcement). Target: lessons-codification / Standing Process Rules. Tracked for next session-wrap or lessons update.
...
- Active worktrees: 1 — STORY-081 in `.worktrees/STORY-081` on `feature/STORY-081` HEAD `46ed9eb9`. Pass 5 NOT CLEAN (1 MED + 1 OBS, zero CRIT/HIGH). Pass-4 fix burst recorded at HEAD 46ed9eb9. Implementer fix burst (ResolvedFace type-invariant) NEXT. Open PRs: 0.
- Workspace: 3759 pass / 20 skip (STORY-081 worktree at HEAD 46ed9eb9; cargo deny PASS; cold_budget PERMANENTLY FIXED by STORY-080 PR#73; linux-arm64 CI has 1 unresolved nextest failure — FU-CI-ARM64-TEST-FAILURE).
...
6. **Continue in priority order:** STORY-081 — Pass 5 NOT CLEAN (HEAD `46ed9eb9`; 1 MED + 1 OBS, zero CRIT/HIGH). Pass-4 fix burst (CRIT-001 + HIGH-001 + size-reduction + docstrings) CONFIRMED load-bearing. Implementer fix burst: refactor `ResolvedFontSet` to `ResolvedFace { font, raw, face_index }` type invariant in font.rs; thread per-span `face_index` into `compute_multi_span_x_positions` → `measure_text_width_pt` (exporter.rs:1239, not hardcoded 0); add non-zero-face-index regression test. Closes BOTH ADV-P05-MED-001 + OBS-P05-001 structurally. Then LESSON-21 exit gate → adversary Pass 6. Pass 5 report: `cycles/STORY-081/adversarial-reviews/adversary-STORY-081-pass-5.md`. Per-story adversary passes SERIAL (LESSON-7 + rate-limit).
...
**Wave 5 IN PROGRESS. develop 15838de1 (79 merged PRs). STORY-081 Pass 5 NOT CLEAN — implementer fix burst (ResolvedFace) NEXT. 1 worktree active. 12 stories remain.**
...
| **Active worktrees** | 1 — STORY-081 in `.worktrees/STORY-081` on `feature/STORY-081` HEAD `46ed9eb9`. |
| **STORY-081 state** | HEAD `46ed9eb9` (Pass-4 fix burst — CRIT-001 + HIGH-001 + size-reduction + docstrings — completed; now checkpointed); adversary 0/3; Pass 5 NOT CLEAN (1 MED + 1 OBS, zero CRIT/HIGH); Pass-4 findings ALL CONFIRMED load-bearing |
| **Pass 5 open findings** | ADV-P05-MED-001: `compute_multi_span_x_positions` hardcodes `face_index=0` in `measure_text_width_pt`; fontdb resolves non-zero face_index for .ttc → drawn face ≠ measured face → mis-spaced multi-span lines. OBS-P05-001: `with_resolved_font_set` seam allows `Some(font)+None raw` → silent zero-advance overprint; codify `tracing::warn` or `ResolvedFace` type invariant. |
| **Workspace tests** | 3759 pass / 20 skip (HEAD 46ed9eb9; cargo deny PASS; linux-arm64 CI: 1 unresolved nextest failure — FU-CI-ARM64-TEST-FAILURE) |
| **factory-artifacts** | Pushed to origin. Fresh sessions: clone + `git worktree add .factory factory-artifacts`. |
| **RESUME INSTRUCTION** | STORY-081 Pass 5 NOT CLEAN (HEAD 46ed9eb9). NEXT: implementer fix burst — refactor `ResolvedFontSet` to `ResolvedFace { font, raw, face_index }` type invariant in font.rs; populate `face_index` from `with_face_data` callback in `resolve_styled_face_via_fontdb`; thread per-span `face_index` into `compute_multi_span_x_positions` → `measure_text_width_pt` (exporter.rs:1239, not 0); add regression test asserting non-zero-index measure differs from index-0 (.ttc or synthetic unit test). Closes ADV-P05-MED-001 + OBS-P05-001 structurally. Then LESSON-21 exit gate → adversary Pass 6. Pass 5 report: `cycles/STORY-081/adversarial-reviews/adversary-STORY-081-pass-5.md`. Per-story: LOCAL adversary 3-CLEAN (SEQUENTIAL) → demo-recorder → rebase onto 15838de1 → pr-manager 9-step → STANDING MERGE AUTH → squash-merge → state-manager post-merge burst → worktree cleanup. Rate-limiting: ONE adversary/review pass at a time. On next PR: CAPTURE linux-arm64 nextest failure test name (FU-CI-ARM64-TEST-FAILURE). HELD next batch: STORY-056/048 (unblocked ←047), STORY-057/058/064 (serialize cli), STORY-060/061 (GIT2-OPENSSL first). |
