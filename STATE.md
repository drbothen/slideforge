...
workspace_tests: "3820 pass / 20 skip / 0 fail (STORY-081 worktree HEAD e5b1e92e; ADR-024 PPTX inline-run generator UNIFICATION complete — single render_inline_nodes_to_runs engine; F-P16-M1 fixed; dead body generator deleted)"
...
- **Pass 16 findings (all REMEDIATED):**
  ...
  - M2[MED] — cross-path parity test asserted only body/notes parity, not absolute per-shape rel/click contract; shared-mode regression would pass silently. REMEDIATED: replaced by ADR-024 Step-7 structural invariant test (INV-4 + INV-5).
  - L1[LOW] — notes link-formatting-loss intent question. RESOLVED: human ruled DEFECT (preserve formatting); no notes-plain carve-out.
