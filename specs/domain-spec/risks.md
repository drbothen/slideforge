...
| R-003 | HTML-to-PDF pipeline via Chrome headless cannot produce PDF/UA-1 compliant tagged PDF | Medium | HIGH | reliability | mitigated — Mitigated by ADR-003: PDF generation uses pdf-writer + krilla with SlideTagEngine for PDF/UA-1 compliance. Chrome headless dependency eliminated. | Research Chrome headless tagged PDF support as part of ASM-007 validation. Mitigation if Chrome fails: switch PDF exporter to pdf-rs or lopdf with direct tagged PDF generation. NFR candidate: yes — veraPDF in CI quality bar. Security focus: no | ASM-007, CAP-017, DI-014 |
