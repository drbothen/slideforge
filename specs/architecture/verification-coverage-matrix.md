...
| VP-053 | Inline HTML: all interpolated values are HTML-escaped in all output positions (text + attribute) | slideforge-plugin-api | proptest | P3 | P1 |
...
| slideforge-plugin-api | 0 | 0 | 1 | 0 | 0 | 1 |
...
- slideforge-plugin-api: VP-053 proptest (HTML-escaping of all interpolated values in all output positions — text + attribute — for DefaultInlineFormat::render(_, Html)). Traces to BC-3.05.001 v1.3.6, EC-009, EC-010. Covers the gap identified when the PO tightened the HTML postcondition.
...
| P1 (stretch / Phase 3+) | 46 |

**Arithmetic check:** 16 (Kani) + 6 (proptest) + 2 (fuzz) + 27 (unit) + 2 (integration) = 53 total. Consistent.

**VP-INDEX cross-check:** VP-INDEX total = 53. Coverage matrix VP row count = 53. Per-tool column totals: 16 + 6 + 2 + 27 + 2 = 53. Consistent.
