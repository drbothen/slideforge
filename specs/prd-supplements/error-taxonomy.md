...
| W-VAL-103 | cosmetic (non-content-drop) / broken (content-drop) | 0 / 2 (strict; content-drop = `shape`\|`body` unknown to slide type; `--warn-only` always 0) | `Unknown field '<key>' for slide type '<type>'. Known fields: [<list>].` | BC-1.18.001 (unknown-field branch), BC-3.03.002, CAP-022, STORY-098 |
...
| 2.33 | 2026-06-11 | product-owner | F-098-P11-001 (MED) — **W-VAL-103 summary-table row synced to match Route A Note (v2.29/v2.32 table-row miss).** The W-VAL-103 summary-table row Severity and Exit cells were stale (`cosmetic | 0`) at v2.29–v2.32 while the Route A Note 44 lines below had been maintained with the correct context-sensitive semantics. Row now reads: Severity `cosmetic (non-content-drop) / broken (content-drop)`, Exit `0 / 2 (strict; content-drop = shape\|body unknown to slide type; --warn-only always 0)`. Traces To expanded to add `BC-3.03.002, STORY-098` (BC-1.18.001 and CAP-022 retained). The sibling E-LAY-003 row was correctly updated at v2.31; no other W-VAL or E-VAL rows in this taxonomy have Route A-equivalent split-severity semantics requiring the same treatment. LESSON-19: summary-table rows must be scanned for staleness in the same burst as any Note-section severity change. |
