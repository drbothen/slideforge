...
| E-PAR-023 | broken | 2 | `section group name must be non-empty at <file>:<line>:<col>. Provide a quoted, non-empty name, e.g. section "Background":` | BC-4.01.003 EC-010, CAP-001 |

Note (E-PAR-023): `ParseError::EmptySectionGroupName` — emitted when the parser encounters a `section "":` slide-grouping block whose quoted-name token is an empty string. An empty section name is rejected at parse time (before IR or GUID derivation occurs), not at eval or validation time: the parser can determine emptiness immediately after consuming the quoted-string token. Severity: broken. Exit 2 in strict mode (consistent with eval-stage errors for section-related constructs; this is a semantic parse error rather than a pure syntax error, because the quoted-string token is syntactically valid but semantically useless). Error is accumulated (parsing continues past this point to find additional errors). No `SectionGroupNode` is produced for the rejected block. The `<file>:<line>:<col>` span points at the opening `"` of the empty name token. Variant name: `ParseError::EmptySectionGroupName`. Owner story: STORY-082. Traces to BC-4.01.003 EC-010, CAP-001.
...
(3) **SEC-001 (CWE-674, nesting-depth cap exceeded, 64-level limit)** — `crates/slideforge-diagrams/src/normalize.rs` and `crates/slideforge-pdf/src/svg_embed.rs` both scan the parsed `usvg::Tree` for `<g>` group nesting depth > `MAX_SVG_NESTING_DEPTH` (64) via iterative stack-based DFS (`Group::children()` matching `usvg::Node::Group`; root counts as depth 1). Cause: `"SVG group nesting depth N exceeds the 64-level limit; possible DoS input"`. The scan is iterative (not recursive) to avoid replacing one stack-exhaustion vector with another.
...
**STORY-043 / STORY-079:** SEC-001/SEC-002 were first introduced in `slideforge-pdf` (STORY-043) and applied to `slideforge-diagrams` (STORY-079) for defense-in-depth. Both crates share the same constant values and cause string wording.
