...
Evaluate → Brand → Validate → Layout → Export) can deliver all P0 BCs within the stated NFR targets. Three
...
| 4 — Export | BC-4.03.005 | EMU to PDF coordinate mapping with Y-axis flip | 1pt = 12700 EMU; krilla Surface is top-left Y-down (surface.rs:44, page.rs:262-263); draw path uses `emu_to_pt(ir_y)` directly; `ir_y_to_pdf_y` is a pure function / VP-006 Kani target, not called at draw time. See BC-4.03.005 v1.2, DIR-044-001. | Feasible | The coordinate mapping is a pure function. Kani proof is specified in BC-4.03.005 and is feasible (bounded integer arithmetic). |
